...
function MikuChat() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      from: "miku",
      text: "Hey master! I'm Miku, your personal nutrition companion 💚 Ask me anything about food, fitness, or just chat!",
    },
  ]);
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((m) => [
        ...
        {
          from: "miku",
          text: "Oops, something went wrong on my end! Try again, master 💦",
        },
      ]);
      setMood("concerned");
    } finally {
  ...
  return (
    <AppShell>
      ...
      <PremiumGate
        ...
        feature="Voice chat with Miku is a Premium feature"
      />
      ...
      <div className="glass rounded-3xl p-5 mb-4 flex flex-col items-center relative overflow-hidden">
        ...
        {isPremium && (
          <div className="mt-1 text-[10px] px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 font-bold">
            💎 Premium
          </div>
        )}
      </div>
      ...
      <div className="glass rounded-3xl flex flex-col h-[420px] overflow-hidden">
        ...
        <div className="border-t border-border/40 p-2 flex items-center gap-2">
          ...
          {/* Voice button — premium gated */}
          <motion.button
            whileTap={{ scale: 0.88 }}
            onClick={() => !isPremium && setGateOpen(true)}
            className={`w-9 h-9 rounded-full grid place-items-center relative transition ${
              isPremium
                ? "bg-amber-500/20 text-amber-300"
                : "bg-muted/40 text-muted-foreground"
            }`}
            title={isPremium ? "Voice chat" : "Voice — Premium"}
          >

...
function Nutrition() {
  ...
  const logMeal = async () => {
    ...
    await supabase.from("meal_logs").insert({
      ...
      kcal: Math.round(selectedFood.kcal_per_100g * g),
      protein: Math.round(selectedFood.protein_per_100g * g),
      carbs: Math.round(selectedFood.carbs_per_100g * g),
      fats: Math.round(selectedFood.fats_per_100g * g),
    });
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        {food.kcal_per_100g} kcal · {food.protein_per_100g}P · {food.carbs_per_100g}C · {food.fats_per_100g}F per 100g
                      </div>
                    </div>
                    {selectedFood?.id === food.id && (
        ...
        {grouped.map(({ type, items }) => (
          <div key={type} className="glass rounded-3xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <motion.button
                whileTap={{ scale: 0.9 }}
                onClick={() => { setAddingTo(type); setSearch(" "); searchFoods("a"); }}
...
function Stat({
  ...
  label: string; v: number; t: number; c: string;
}) {
